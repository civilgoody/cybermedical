...
const fetchDashboardData = async (): Promise<Report[]> => {
  ...
  try {
    ...
    console.log("✅ Dashboard data fetched successfully via API:", {
      count: data?.length || 0,
    });
...
const processAttackFrequency = (
  ...
  const filteredReports = reports.filter(
    (report) => new Date(report.created_at) >= startTime
  );

  // Use different bucket sizes based on timeframe
  const bucketData = new Map<number, { label: string; count: number }>();
  ...
  if (timeframe === "24h") {
    ...
    filteredReports.forEach((report) => {
      ...
      const entry = bucketData.get(timestamp) || { label, count: 0 };
      entry.count++;
      bucketData.set(timestamp, entry);
    });
  } else {
    // Daily buckets for 7d and 30d views
    filteredReports.forEach((report) => {
      const date = new Date(report.created_at);
...
const processSeverityBreakdown = (
  reports: Report[],
  timeframe: "24h" | "7d" | "30d"
) => {
  const now = new Date();
  ...
  if (timeframe === "24h") {
    ...
    filteredReports.forEach((report) => {
      ...
      const severity = report.severity as
        | "low"
        | "medium"
        | "high"
        | "critical";
